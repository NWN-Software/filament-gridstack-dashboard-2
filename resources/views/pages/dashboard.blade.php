...
<x-filament-panels::page class="fi-dashboard-page">
    @if ($designMode)
        <div x-ignore x-load
            x-load-src="{{ FilamentAsset::getAlpineComponentSrc('filament-gridstack-dashboard-script', 'invaders-xx/filament-gridstack-dashboard') }}"
            x-data="gridStackDashboard({
                ...
                resizable: '{{ $resizable }}'
            })" x-load-css="[@js(FilamentAsset::getStyleHref('filament-gridstack-dashboard-styles', package: 'invaders-xx/filament-gridstack-dashboard'))]" class="text-center">
            ...
            @foreach ($this->buildGridItemsForDesign() as $row => $widgets)
                <div class="grid grid-cols-{{ $columns }} gap-6">
                    @foreach ($widgets as $widgetKey => $widget)
                        @if ($widget['id'] === null)
                            <div class="col-span-{{ $widget['w'] }} fi-wi-widget" default="{{ $widget['w'] }}">
                            </div>
                        @else
                            @php
                            ...
                            @endphp
                            <div class="col-span-{{ $widget['w'] }} fi-wi-widget" :default="{{ $widget['w'] }}">
                                @livewire($widgetClass, [...$widget['id'] instanceof \Filament\Widgets\WidgetConfiguration ? [...$widget['id']->widget::getDefaultProperties(), ...$widget['id']->getProperties()] : $widget['id']::getDefaultProperties(), ...$data], key("{$widgetClass}-{$widgetKey}"))
                            </div>
                        @endif
                    @endforeach
                </div>
